...
async function getTower1RankingHandler(req, reply) {
  let pool;
  try {
    ...
    const rankingResult = await pool.request()
      .query(`
        SELECT TOP 5 [cName], [cStageGroupHash], [cLevel], [cTime]
        FROM [v4_stage].[dbo].[TowerRank]
        WHERE cStageGroupHash = '19141002'
    ...
    req.log.info('Ranking da torre 1 retornado com sucesso');
...
module.exports = function (fastify, opts, done) {
  fastify.get('/tower1Ranking', getTower1RankingHandler);
  done();
};
